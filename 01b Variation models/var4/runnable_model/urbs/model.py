                ...
                cap_pro = \
                    (sum
                     (m.cap_pro_new[stf_built, sit, pro]
                      for stf_built in m.stf if (sit, pro, stf_built, stf) in m.operational_pro_tuples) #sum over all that still exist
                     + m.process_dict['inst-cap'][(min(m.stf), sit, pro)] # + initial value
                     ) \
                    - sum(m.cap_decommissioned[stf_dec, sit, pro] for stf_dec in m.stf if stf_dec <= stf if stf_dec > min(m.stf) if (stf_dec, sit, pro) in m.pro_decom_cap_dict) #- sum of decommissioned
